...
def normalize_amplitude(signal, target_dBFS=-20.0):
    ...
    if np.isclose(rms, 0.0, atol=np.finfo(float).eps):
...
def peak_normalize(signal, target_peak=0.95):
    ...
    if np.isclose(peak, 0.0, atol=np.finfo(float).eps):
...
def z_normalize(signal):
    ...
    if np.isclose(std, 0.0, atol=np.finfo(float).eps):
...
def min_max_normalize(signal, min_val=0.0, max_val=1.0):
    ...
    if np.isclose(current_min, current_max, atol=np.finfo(float).eps):
